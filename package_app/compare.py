...
def compare_versions(version1, version2):
    return rpm.labelCompare(version1, version2)
...
def compare_packages(first_branch_data: PackageData, name_first_branch: str, 
    ...
    results_by_arch = {
        arch: {
            ...
            f'ver-release_packages_more_{name_second_branch}_than_{name_first_branch}': [pkg for pkg in common_packages if packages_from_second_branch[pkg].arch == arch and compare_versions(packages_from_second_branch[pkg].release, packages_from_first_branch[pkg].release) > 0]
        }
